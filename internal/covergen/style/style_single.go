...
func resizeAndCropImage(img image.Image, targetWidth, targetHeight int) image.Image {
	...
	cropRect := image.Rect(offsetX, offsetY, offsetX+cropWidth, offsetY+cropHeight)
...
func blendImages(background, foreground image.Image, ratio float64) image.Image {
	...
	for y := bounds.Min.Y; y < bounds.Max.Y; y++ {
		for x := bounds.Min.X; x < bounds.Max.X; x++ {
			...
			result.Set(x, y, stdcolor.RGBA{R: r, G: g, B: b, A: 255})
		}
	}
...
func DrawTitles(img image.Image, mainTitle, subTitle string, 
	...
	bottomPadding := int(bounds.Dy() * 0.15)
	...
	if zhFont != nil {
		...
		ctx.SetDst(result)
		ctx.SetSrc(image.NewUniform(stdcolor.White))
